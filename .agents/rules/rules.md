...
- **Lasttests niemals eigenständig starten.** KI-Agenten dürfen `pnpm spike`, `pnpm spike:report`, direkte `k6`-Läufe oder andere Last-/Kapazitätstests ausschließlich nach einer ausdrücklichen, aktuellen Freigabe des Nutzers für genau diesen Lauf starten. Ein Todo, Phasenabschluss oder allgemeiner Implementierungsauftrag ist **keine** Freigabe. Ohne Freigabe dürfen nur bestehende Lasttest-Artefakte gelesen und die Testkommandos zur manuellen Ausführung genannt werden.
