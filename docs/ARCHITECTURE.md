...
6. API (Pay-Route) validiert das (simulierte) Payment-DTO und claimt den Reservierungs-Record per Lua atomar `pending → publishing` (inkl. `queuedAt = Date.now()`). Nur der Gewinner dieses Claims published den `BuyTicketEvent` an Pub/Sub. Nach bestaetigtem Publish markiert ein zweites zustandsbewusstes Script `publishing → paid`; der Worker ersetzt diesen Zustand spaeter durch sein finales Read-Model. `publishing|paid` werden im oeffentlichen Status weiterhin als `pending` dargestellt, damit Clients bis `completed|failed` weiterpollen.
   ❌ Publish-Fehler → ein atomares Gegen-Script gibt nur den eigenen `publishing`-Claim frei: `ZREM reservations {orderId}`, `INCR available` nur wenn der Ledger-Eintrag tatsaechlich noch existierte (idempotent, kein Double-Increment), `DEL` Order-State.
   ↩︎ Bricht der Nutzer das Modal ab / laeuft 3DS aus → POST /api/orders/{orderId}/cancel gibt ausschliesslich `pending` frei. Gewinnt Pay den Race, sieht Cancel den Zustandskonflikt und darf den publizierenden Anspruch nicht freigeben.
