...
- **Entscheidung:**
  1. `oliver006/redis_exporter` als Docker-Compose-Service (`hfts-redis-exporter`, Host-Port `10009`, Container-Port `9121`) mit eigenem Prometheus-Scrape-Job (`job: redis`, container-intern per Service-Name). Aktiviert die bestehenden Redis-Performance-Panels.
