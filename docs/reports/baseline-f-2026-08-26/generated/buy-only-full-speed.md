# Load-Test Report — `2026-08-26T10-35-15-726Z-6e2687b`
...
- **Configuration (load harness / orchestrator env):**
  ...
  - `CANCEL_RATE` = `0`
  - `CHECKOUT_PENDING_TIMEOUT_SECONDS` = `900`
  - `CHECKOUT_SHARE` = `1`
  - `DATABASE_POOL_CONNECTION_TIMEOUT_MS` = `5000`
  - `DATABASE_POOL_MAX` = `50`
  - `DISABLE_REQUEST_LOGGING` = `true`
  - `HTS_ENV_PROFILE` = `buy-only-full-speed`
  - `K6_COOLDOWN_MAX_VUS` = `5000`
  - `K6_COOLDOWN_RATE` = `1000`
  - `K6_MAX_VUS` = `10000`
  - `K6_RUNNER` = `ssh`
  - `K6_TARGET_RATE` = `5000`
  - `LOAD_PROFILE` = `buy-only-full-speed`
  - `LOG_LEVEL` = `warn`
  - `NODE_ENV` = `production`
  - `PAY_RATE` = `1`
  ...
  - `WORKER_RESERVATION_REAPER_BATCH_SIZE` = `1000`
...
- Dropped-iteration rate 2.40% exceeds the valid-run warning limit.
...
| --------- | ---------- | ------- | --------- | ---------- | ------- |
| phase-a   | 1 141 114  | 29 557  | 1 170 671 | 97.48%     | 10 000  |
| phase-b   | 60 000     | 0       | 60 000    | 100.00%    | 200     |
| **total** | 1 201 114  | 29 557  | 1 230 671 | 97.60%     |         |

- **Phase A stop reason:** `sold-out` (available at stop: 0)

- **Transport errors (phase-a):** 15 640 — buy 15 640
...
- **Mean:** 0.092s (Δsum/Δcount)
- **p50 / p95 / p99:** 0.100s / 0.250s / 0.500s
...
- **Redis:** available 0, active reservations 0, keys 2 000 003, used memory 555 302 456 bytes
...
- **generator-saturated:** Load generator dropped scheduled iterations; do not claim the target RPS as backend capacity.
  - evidence: `droppedShare=0.024016979355164784`
