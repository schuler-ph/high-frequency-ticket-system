...
// ADRs und Reports beschreiben bewusst auch Dateien, die es nicht mehr gibt
// ("der bisherige X", "gemessen an Zeile Y"). Solche historischen Erwaehnungen sind
// kein toter Verweis und werden hier freigestellt. Nur diese beiden Ablagen sind
// per Definition unveraenderlich (siehe docs/DOCS.md) — aktuelle Quellen der
// Wahrheit muessen ihre Pfade weiterhin aktuell halten.
const HISTORICAL_DIRS = ["docs/decisions/", "docs/reports/"];
const HISTORICAL_PATHS = new Set([
  ...
  "apps/worker/src/lib/reconcile-ticket-availability.ts", // ADR-031: 2026-07-30 durch Auditor/Projector/Reaper ersetzt
]);

const isHistoricalMention = (relFile, target) =>
  HISTORICAL_DIRS.some((dir) => relFile.startsWith(dir)) &&
...
for (const absFile of [
  ...
  lines.forEach((line, index) => {
    ...
    for (const match of line.matchAll(BACKTICK_PATH)) {
      ...
      if (isHistoricalMention(relFile, target)) {
        continue;
      }
